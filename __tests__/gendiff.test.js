...
describe('yaml', () => {
  ...
  test('difference for files with nested structure with formatter plain', () => {
    const __dirname = dirname(fileURLToPath(import.meta.url));
    const result = genDiff(resolve(__dirname, '../__fixtures__/case1/file-3.yml'), resolve(__dirname, '../__fixtures__/case1/file-4.yaml'), 'plain');
    expect(result).toBe(
      `Property 'common.follow' was added with value: false
Property 'common.setting2' was removed
Property 'common.setting3' was updated. From true to null
...
Property 'group3' was added with value: [complex value]`,
  ...
  test('difference for files with nested structure', () => {
    const __dirname = dirname(fileURLToPath(import.meta.url));
    const result = genDiff(resolve(__dirname, '../__fixtures__/case1/file-3.yml'), resolve(__dirname, '../__fixtures__/case1/file-4.yaml'), 'stylish');
    expect(result).toBe(
      `{
    common: {
      + follow: false
